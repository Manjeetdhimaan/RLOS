    ...
    modelToDTO(model, applicant) {
        ...
        if (model.basicDetails) {
            applicant.prefix = model.basicDetails.prefix,
                ...
                applicant.dob = model.basicDetails.dob ? model.basicDetails.dob.getFullYear() + '-' + this.formatDate(model.basicDetails.dob.getMonth() + 1) + '-' + this.formatDate(model.basicDetails.dob.getDate()) : null,
                applicant.idExpDate = model.basicDetails.idExpDate ? model.basicDetails.idExpDate.getFullYear() + '-' + this.formatDate(model.basicDetails.idExpDate.getMonth() + 1) + '-' + this.formatDate(model.basicDetails.idExpDate.getDate()) : null,
                applicant.idIssueDate = model.basicDetails.idIssueDate ? model.basicDetails.idIssueDate.getFullYear() + '-' + this.formatDate(model.basicDetails.idIssueDate.getMonth() + 1) + '-' + this.formatDate(model.basicDetails.idIssueDate.getDate()) : null,
                applicant.ssn = this.updateSSNFormat(model.basicDetails.ssn),
                applicant.idType = model.basicDetails.idType,
                applicant.idNumber = model.basicDetails.idNumber,
                ...
                applicant.isAlreadyExist = false;
        ...
        if (model.addressDetails) {
            ...
            applicant.homePhoneNo = model.addressDetails.homePhoneNo;
            applicant.mobileNo = model.addressDetails.mobileNo;
            applicant.homePhoneType = model.addressDetails.homePhoneType;
            applicant.altPhoneType = model.addressDetails.altPhoneType;

            addressDetails.addresses = [];
            if (model.addressDetails.currentAddress && model.addressDetails.currentAddress.city) {
            ...
            if (model.addressDetails.mailingAddress && model.addressDetails.mailingAddress.city && addressDetails.isMailingAndResidentialAddDifferent) {
            ...
            if (model.addressDetails.previousAddress && model.addressDetails.previousAddress.city && model.addressDetails.currentAddress.durationInMonths < 24) {
                // model.addressDetails.previousAddress.durationInMonths = this.convertToMonths(model.addressDetails.previousAddress.years, model.addressDetails.previousAddress.months);
                addressDetails.addresses.push(Object.assign({ type: "PREVIOUS" }, model.addressDetails.previousAddress));
            }
            // if (model.addressDetails.previousAddresses) {
            //     model.addressDetails.previousAddresses.forEach((prevAddress) => {
            //         addressDetails.addresses.push(Object.assign({ type: "PREVIOUS" }, prevAddress));
            //     });
            // }
            applicant.addressDetails = addressDetails;
        }
